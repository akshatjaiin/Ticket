...
fetch(url, options)
  ...
  .then((data) => {
    if (data && data.response && data.response.length > 0) {
      ...
      // Use textContent for label and text, and append elements safely
      chatDiv.innerHTML = `
        <div class="chat response">
          <button class="text-to-speech" onclick="speak(this)">
            <svg version="1.0" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="1em" height="1em" viewBox="0 0 64 64" enable-background="new 0 0 64 64" xml:space="preserve">
          ...
          <label for="response">Bot</label>
          <p name="response" class="ai_response"></p>
        </div>
      `;

      // Safely insert the AI response
      chatDiv.querySelector('.ai_response').textContent = aiResponse.your_response_back_to_user;

      // Update the language field its backends job
